...
class GanServer(fl.server.strategy.FedAvg):
    ...
    def aggregate_evaluate(
        ...
        for _, eval_res in results:
            ...
            accuracy += metrics.get("accuracy", 0.0) * n
            precision += metrics.get("precision", 0.0) * n
            recall += metrics.get("recall", 0.0) * n
            f1 += metrics.get("f1_score", 0.0) * n
        ...
        aggregated_metrics = {
            ...
            "f1_score": f1 / total_examples,
        }
        # Explicitly log evaluation metrics for visibility
        logger.info(
            f"Round {server_round} evaluation metrics: "
            f"accuracy={aggregated_metrics['accuracy']:.4f}, "
            f"precision={aggregated_metrics['precision']:.4f}, "
            f"recall={aggregated_metrics['recall']:.4f}, "
            f"f1_score={aggregated_metrics['f1_score']:.4f}"
        )
